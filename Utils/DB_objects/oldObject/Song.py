...
class Song(DB_common_object):
    ...
    def __get_song(cls, data):
        coup_req = (
            f"SELECT C.id, C.name, C.text "
            f"FROM Song_Couplet SC "
            f"LEFT JOIN Couplet C ON SC.couplet_id = C.id "
            f"WHERE SC.song_id = {data['song_id']} "
            f"ORDER BY SC.number"
        )
    ...
    def __search_song(cls, data):
        ...
        if any((c in '1234567890') for c in song_search_str):
            found_id = cls.exec_req(
                f"SELECT S.id FROM Song S "
                f"WHERE S.number = {int(song_search_str)}"
            ).fetchall()
        else:
            found_id = cls.exec_req(
                f"SELECT S.id FROM Song S "
                f"WHERE S.name_upper "
                f"LIKE \"%{song_search_str.upper()}%\""
            ).fetchall()
        if not len(found_id):
